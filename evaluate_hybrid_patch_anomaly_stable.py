...
def score_dataset(
    ...
    *,
    ...
    for i in range(X.shape[0]):
        ...
        S = structure_residual_map(img, rec, sigma_small=cal.sigma_small, sigma_large=cal.sigma_large) * msk
        ...
        if block_mu is not None and block_sig is not None:
            ...
            vv = valid_mask.reshape(-1)
            rar_scores[i] = float(np.mean((z.reshape(-1)[vv]) ** 2))
        else:
            rar_scores[i] = 0.0
    ...
    # latent z-norm (split-wise) — keeps ranking stable
